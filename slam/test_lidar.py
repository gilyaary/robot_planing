...
with Serial ( '/dev/serial0' , 230400 ) as ser :
    ...
    try :
        ...
        while True :
            ...
            # if the byte is the header of the packet and it is the zero element of the buffer if startCount == 0 and buff [ startCount ] == HEADER : # change the index 
            startCount = 1
            # if the index is changed and the byte read is the first packet if startCount == 1 and buff [ startCount ] == FIRSTPACKET : # reset the index 
            startCount = 0
            ...
            while i < BUFFSIZE :
                ...
                i += 1
                # read each package for i in range ( 0 , BUFFSIZE , PACKETSIZE ):
                # if the packet starts with a header and the packet index is correct if buff [ i ] == HEADER and buff [ i + 1 ] == int ( FIRSTPACKET + i / PACKETSIZE ):
                # calculate data in batch for j in range ( i + DATASTART , i + DATAEND , OFFSETSIZE ):
                ...
                # output data print ( "r[{}]={}" . format ( 359 - index , distance / 1000 ))
    finally :
