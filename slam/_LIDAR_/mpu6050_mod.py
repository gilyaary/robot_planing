...
if __name__ == "__main__":
    #68 is the identifier of this chip in i2s. this is what we see when we do sudo i2cdetect -y 0
    ...
    while True:
        ...
        #From gyro we only need the yaw (z). From accel we need x,y. (z - would be gravity i.e ~ 1)
        #print(f"{x:.6}")
        _yaw = gyro_data['z']
        _x = accel_data['x']
        _y = accel_data['y']
        ...
        if count != -1:
            ...
            discount = (count-1) / count
            yaw = discount * (yaw + _yaw)
            x = discount * (x + _x)
            y = discount * (y + _y)
        print("Yaw:", f"{yaw:.2}", "X:", f"{x:.2}","Y:", f"{y:.2}", "Z:", f"{z:.2}")
        time.sleep(2)
