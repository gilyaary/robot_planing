...
N_PARTICLE = 1000  # number of particle
...
def motion_model(x, u):
    ...
    #distance_x = (dtime * velocity) * cos(theta)
    #distance_y = (dtime * velocity) * sin(theta)
...
'''
The motion model takes current location and linear and angular velocity and returns 
the new x,y,theta location
'''
def __main():
    #def motion_model(x, u):
    ...
    #distance_x = (dtime * velocity) * cos(theta)
    #distance_y = (dtime * velocity) * sin(theta)
    B = np.array([[TIME_DELTA * math.cos(x[2, 0]), 0],
                  [TIME_DELTA * math.sin(x[2, 0]), 0],
                  [0.0, TIME_DELTA]])
    ...
    # change current angle by the change of angular_velocity * time_delta:
...
def _main():
    ...
    while SIM_TIME >= time:
        ...
        #xTrue: 3*1 array of x,y,theta
        #z distance, angle, index (obs with max range are ommitted)
        #u [[velocity, yaw_rate]]
...
def main():
    number_of_particles = N_PARTICLE
    number_of_landmarks = 2
    particles = [Particle(number_of_landmarks) for _ in range(number_of_particles)]
    v0 = 1.0  # [m/s]
    v1 = 1.0  # [m/s]
    yaw_rate = 0.0 * math.pi  # [rad/s]
    u0 = np.array([v0, yaw_rate]).reshape(2, 1)
    u1 = np.array([v1, yaw_rate]).reshape(2, 1)
    #z distance, angle, index (obs with max range are ommitted)
    z = np.zeros((3, 0))
    zi = np.array([3, pi_2_pi(math.pi*0.0), 0]).reshape(3, 1)
    z = np.hstack((z, zi))
    zi = np.array([3, pi_2_pi(math.pi*0.5), 1]).reshape(3, 1)
    z = np.hstack((z, zi))



    #First assum velocity = 0 (u0)
    for i in range (0, 10):
        particles = fast_slam1(particles, u0, z)
